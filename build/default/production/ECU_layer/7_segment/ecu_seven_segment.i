...
Std_ReturnType seven_seg_initialize(const seven_seg_t *seven_segment){
    ...
    {
        ret = gpio_pin_intialize(&(seven_segment->seven_seg_pin[0]));
        ret = gpio_pin_intialize(&(seven_segment->seven_seg_pin[1]));
        ret = gpio_pin_intialize(&(seven_segment->seven_seg_pin[2]));
        ret = gpio_pin_intialize(&(seven_segment->seven_seg_pin[3]));
    }
    return ret;
}
# 37 "ECU_layer/7_segment/ecu_seven_segment.c"
Std_ReturnType seven_seg_write_number(const seven_seg_t *seven_segment, uint8_t number){
    Std_ReturnType ret = (Std_ReturnType)0x01;
    if((((void*)0) == seven_segment)||(number > 9))
    {
        ret = (Std_ReturnType)0x00;
    }else
    {
        ret = gpio_pin_write_logic(&((seven_segment->seven_seg_pin[0])), number & 0x01);
        ret = gpio_pin_write_logic(&((seven_segment->seven_seg_pin[1])), (number>>1) & 0x01);
        ret = gpio_pin_write_logic(&((seven_segment->seven_seg_pin[2])), (number>>2) & 0x01);
        ret = gpio_pin_write_logic(&((seven_segment->seven_seg_pin[3])), (number>>3) & 0x01);
    }
    return ret;
}
